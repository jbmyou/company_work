...
# 세부종류에 해당하는 것은 1에 써넣어서 세부종류 문구를 살린다.
        ...
        "개인정보1" : re.compile(r"신분증|운전면허|주민등록증|^(?!.*(신용회복|채무조정안)).*(기초)?수급자?|^(?!.*(신용회복|채무조정안)).*차상위|(법인|사용)?\s?인감|기본\s?증명서?|(가족|혼인)\s?(관계|증명)|이혼|입양|친양자|졸업|병적|장애인(등록)?증|복지카드"),
        # 등초본
        ...
        # 키워드 추가(대체x)
        "기타" : re.compile(r"기타"),
        "기타1" : re.compile(r"집행문\s?부여|배송[가-힣]+|(채권|양도|양수|매매).*계약|화해(?!권고)|대위변제|분할|분납|(?<!미)감면|(상속)?\s?한정\s?승인|\
            ...
            |소장|진단서|사용\s?증명원|해제\s?통지서|의견\s?청취서|느단|즈기|추심\s?요청서?|안내문?|개문|채무\s?이행\s?통지|신탁(해지|해제)?통지서?|최고장|사실조회|잔액증명|채무조정서"), # 제거가 아니므로 첫 글자만 잘 찾으면 된다.
        ...
        "파산" : re.compile(r"파산|파산.*면책"), #파산에도 배당있음. 강제집행보다 먼저 나와야
        ...
        "재산조사1" : re.compile(r"(?<!법인)\s등기|(?<!법인)등기|가압류\s?물건지|.*대장|.*등록\s?원부|.*은행거래|.*입출금"), # 3채무자 등기부등본
...
def rm_s(stem:str) :
    ...
    p_s = re.compile(r"^[_\s)]+|[_\s(]+$|[_\s]{1,}") #(로 끝나거나 )로 시작하는 경우는 제거. 짝 안 맞는 건 주)가 있으니 그냥 두자..
...
def eventFnc(noKeyStem:str, testMode :bool = False)->list:
    ...
    # y-es-sn
    p_event1=re.compile(r"((?<=\D)|^)(?P<y>19\d\d|20"+year_com+")\s?(?P<es>준?재?[가간금나느다머차카타즈본징하개회라정][가-힣]{0,2})\s?(?P<sn>[0-9]+)") # 4자리 연도
    p_event2=re.compile(r"((?<=\D)|^)(?P<y>[7-9][0-9]|"+year_com+")\s?(?P<es>준?재?[가간금나느다머차카타즈본징하개회라정][가-힣]{0,2})\s?(?P<sn>[0-9]+)") # 2자리 연도
    # y-es
    # "\\b" 를 하거나 r"\b"를 해야함
    p_event3=re.compile(r"((?<=\D)|^)(?P<y>19\d\d|20"+year_com+")\s?(?P<es>준?재?[가간금나느다머차카타즈본징하개회라정][가-힣]{0,2})\\b") #반드시 띄어쓰기를 하거나 끝이나야.(& 1보다 먼저 if문에서 나오면 안됨.)
    p_event4=re.compile(r"((?<=\D)|^)(?P<y>[7-9][0-9]|"+year_com+")\s?(?P<es>준?재?[가간금나느다머차카타즈본징하개회라정][가-힣]{0,2})\\b") #반드시 띄어쓰기를 하거나 끝이나야.(& 1보다 먼저 if문에서 나오면 안됨.)
    p_court = re.compile(r"[가-힣]*(법원|지원|지법|서울|대전|대구|부산|광주|수원|\
    ...
    dict = { #사건구분 검색어 / 삭제할 문서구분 키워드
        "집행권원" : [re.compile(r"가[합단소]|나|다|머|차"), comp["집행권원"]],
        "강제집행" : [re.compile(r"카(?!경|기|확)|타|즈(?!기)|본|금|징|가|정[가|명|라|마|고|드|브|스|기]"), comp["강제집행"]], #카경: 결정,판결경정, 카기, 즈기: 기타민사신청(심판경정), 본(접수증)은 강제집행맞다.
        "개인회생" : [re.compile(r"개|회"), comp["개인회생"]], # 라 : 민사항고사건
        "파산" : [re.compile(r"하"), comp["파산"]], 
        "경정" : [re.compile(r"카경|카기전|즈기"), re.compile('집행권원|강제집행')],
        "항고" : [re.compile(r"라"), re.compile('집행권원|강제집행|개인회생|파산|기타')],
        "기타" : [re.compile(r"느|카(기(?!전)|담|확)"), re.compile(r"기타")] #느(단) : 한정상속, 상속포기, 기:의사표시공시송달, 담:담보취소, 확:소송비용확정
        }
